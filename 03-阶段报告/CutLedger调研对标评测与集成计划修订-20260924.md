...
## 十五、闸门闭环首次“拦→修→过”全链路实证（2026-09-26）

第十四节留下的真缺陷（proof0925 速度分层 FAIL：前半 25 镜全 1.55）已根治，系统首次完成 **闸门拦截→根因诊断→修复→重跑→7/7 ACCEPT** 全闭环：

- **根因（数据诊断非猜测）**：`speed = 0.28 + 0.21×dens` 在 dens≥6 饱和钉死 1.55；1_hot40s 前半 onset 密集→整段塌成单档（director_script.yaml 实锤：前半 25 镜 speed 分布 {1.55: 25}）
- **修复（吸取 run26 抹平教训）**：`ai/production_director.py` 新增“速度分层补丁”——只在某半程档位<3 时按 dens **分位数**重映射到 0.55~1.55（单调性保留：密处仍快；已丰富的半程不动）；插在帧网格对齐之前（speed 必须在网格计算前定稿）
- **验证**：proof0926 同料重跑（1348s）→ 前半档位 **1→21**、后半 4 档保持 → 七关闸门 **7/7 全过 ACCEPT**；成片 h264 1080p 19.41s 16MB sha1=1fd5f700cbd307be，output_registry 登记+manifest
- **附带观察**：本轮管线自修复链也在真实工作（切点自检 FAIL→修复轮重渲→复检；LLM 401 两次→回退链路未阻断出片）
- beat_sync_edit 卡证据链追加 proof0926（拦→修→过全链路入证）
