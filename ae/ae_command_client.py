"""AE 命令客户端（op/params 协议）— 权威实现。

本模块为生产 AE 命令通道的权威实现（迁移自 bridges/ae_mcp_client.py）：
- 协议：写入项目根目录 ``.ae-mcp-bridge/ae_command.json``（``{command, args}``），
  与 AE 侧监听脚本（ae_mcp_auto_listener.jsx / ae_mcp_process_command.jsx）匹配；
- 看门狗：写入 ``ae_trigger.json`` 触发 AE 侧轮询；
- 签名：HMAC-SHA256 可选签名（密钥文件 ``.ae-mcp-bridge/.mcp_secret``）。
...
# 迁移后与命令文件同目录（项目根/.ae-mcp-bridge/），与原 bridges 版语义一致
SECRET_FILE = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), ".ae-mcp-bridge", ".mcp_secret"
)
...
class AECommandClient(AEBridgeClient):
    ...
    v2.0: 接入中间件管道（LoggingMiddleware + MetricsMiddleware），
    自动获得日志/指标/重试能力。响应签名验证默认启用。
    """
    ...
    def __init__(
        ...
        super().__init__(
            command_file=command_file or os.path.join(
                os.path.dirname(os.path.dirname(__file__)), ".ae-mcp-bridge", "ae_command.json"
            ),
            result_file=result_file or os.path.join(
                os.path.dirname(os.path.dirname(__file__)), ".ae-mcp-bridge", "ae_result.json"
            ),
            timeout=timeout,
    ...
    def _setup_default_pipeline(self) -> None:
        """创建默认中间件管道: LoggingMiddleware + MetricsMiddleware。"""
        _try_import_core()
        if _MiddlewarePipeline is None:
            logger.debug("MiddlewarePipeline not available, skipping default setup")
            return
        from ae.bridge_middleware import LoggingMiddleware, MetricsMiddleware
        ...
        logger.debug("Default middleware pipeline setup: Logging + Metrics")

    def add_middleware(self, middleware: Any) -> None:
        """添加自定义中间件到管道。

        Args:
            middleware: Middleware 实例（需实现 process_command / process_response）
        """
        if self._pipeline is None:
            _try_import_core()
            if _MiddlewarePipeline is None:
                logger.warning("MiddlewarePipeline not available, cannot add middleware")
                return
    ...
    def send_command(self, op: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        self.clear_result()
        self.clear_command()  # 清除旧命令，避免 processed 标志干扰
        ...
        if self._pipeline is not None:
            return self._send_via_pipeline(command)
        else:
    ...
    def _send_via_pipeline(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def _execute_send(cmd: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # v2.0: 验证响应签名（签名失败仅记录警告，不阻断）
            if self.signature_enabled and self.secret:
                if not self._verify_response_signature(result):
                    logger.warning(
                        f"Response signature verification failed for command: {cmd.get('command')}"
                    )
            return result

        try:
            return self._pipeline.process_command(command, _execute_send)
        except Exception as e:
            logger.error(f"Middleware pipeline error: {e}")
            # 降级：绕过中间件直接执行
            self._write_command_file(command)
            self._trigger_watchdog()
            return self._wait_for_result()
    ...
    def _trigger_watchdog(self):
        """触发看门狗机制，通知 AE 有新命令需要处理"""
        try:
            # 写入触发文件
            trigger_file = os.path.join(
                os.path.dirname(os.path.dirname(__file__)),
                ".ae-mcp-bridge", "ae_trigger.json"
            )
            import json
            trigger = {
                "timestamp": time.time(),
