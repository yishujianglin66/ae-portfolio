...
class AEPParser:
    ...
    def _extract_paths_from_text(self, text: str, context: str, pos: int) -> bool:
        # 快速预过滤：必须有盘符路径(A:\)或 UNC(\\\\)
        if not _PATH_DRIVE_RE.search(text):
            return False
        found = False
    ...
    def _global_supplement_scan(self) -> None:
        ...
        对大文件(>5MB)跳过以避免性能问题，主循环已覆盖关键数据。
        """
        if self.file_size > 5 * 1024 * 1024:
            return  # 大文件跳过，主循环已提取 matchName 和路径
        try:
