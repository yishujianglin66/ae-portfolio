...
class AEAgentPipeline:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.log_warning(f"配置读取失败，回退默认配置: {e}")
            self._config = {
                "audio": {"default_sample_rate": 22050},
                "video": {"default_fps": 30, "sample_interval": 5},
                "output": {"default_dir": "./output"},
    ...
    def _subscribe_events(self):
        ...
        # 可观测性辅助函数在本方法内的事件回调中被引用，须在本作用域可见
        # （修复 F821：原仅在 _init_core_infrastructure 内局部导入，回调作用域不可见）。
    ...
    def _resolve_video_analyzer(self):
        """确定性解析视频分析器实现。

        历史上此处使用裸 `from video_analyzer_enhanced import ...`，
        但该模块已迁移到 scripts/archive/（不在 sys.path），导致
        self.video_analyzer 永久为 None、感知层视频分析静默失效。

        解析顺序（三者 analyze_video() 接口完全一致）：
          1. core.video_analyzer_accelerated.AcceleratedVideoAnalyzer（多进程/GPU 加速）
          2. 扁平导入 video_analyzer_enhanced.EnhancedVideoAnalyzer（若在 sys.path）
          3. 按文件路径确定性加载 scripts/archive/video_analyzer_enhanced.py
        """
        try:
            from core.video_analyzer_accelerated import AcceleratedVideoAnalyzer
            return AcceleratedVideoAnalyzer(), "AcceleratedVideoAnalyzer"
        except ImportError:
            pass

        try:
            from video_analyzer_enhanced import EnhancedVideoAnalyzer
            return EnhancedVideoAnalyzer(), "EnhancedVideoAnalyzer"
        except ImportError:
            pass

        import importlib.util as _ilu
        _root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _archived = os.path.join(_root, "scripts", "archive", "video_analyzer_enhanced.py")
        if os.path.isfile(_archived):
            try:
                _spec = _ilu.spec_from_file_location(
                    "archived_video_analyzer_enhanced", _archived
                )
                _mod = _ilu.module_from_spec(_spec)
                _spec.loader.exec_module(_mod)
                return _mod.EnhancedVideoAnalyzer(), "EnhancedVideoAnalyzer(archived)"
            except Exception as e:  # noqa: BLE001 - 加载归档实现失败不应中断流水线
                self.log_warning(f"归档视频分析器加载失败: {e}")

        return None, None

    def _init_analyzers(self):
        analyzer, impl_name = self._resolve_video_analyzer()
        if analyzer is not None:
            self.video_analyzer = analyzer
            self.log_info(f"视频分析器加载成功: {impl_name}")
        else:
            self.log_warning("视频分析器加载失败: 无可用实现")
    ...
    def _init_phase5_modules(self):
        ...
        # 修复：使用 PersistentLearningLoop 而非内存版 LearningLoop，让历史学习数据
        # (case-store.json / default-value-store.json) 真正可被读取复用。
        try:
            from learning.persistent_learning_loop import PersistentLearningLoop
            self._learning_loop = PersistentLearningLoop()
            self.log_info(
                f"PersistentLearningLoop 已加载 (Phase5 学习循环, "
                f"records={len(self._learning_loop._execution_records)}, "
                f"templates={len(self._learning_loop._case_store.get_all_templates())})"
            )
        except ImportError as e:
            self._learning_loop = None
            self.log_warning(f"PersistentLearningLoop 加载失败: {e}")
