...
| 木偶动画 | ✅ look_at 正立**已固化进引擎**（2026-09-20）；主线其余项仍进行中 | ⚠️ 2026-09-20 核实更正：此前"已突破"与实况不符——仓库里只有孤立桩（`stabilize_camera_rotation` 忽略入参返回常量、两个 `*_calculated/rotation_stabilized` 标志硬编码 True 且函数从未被调用、`puppet/output/` 无产物），配套测试还把桩当规格锁住。现改为真实现（世界 up 投影到 ⟂视线 平面 ⇒ 无滚转）+ 不变量验收（轨道扫掠 360 例：正交性 1.1e-16、地平线偏差 3.2e-15°、指向误差 0；退化输入不产生 NaN）+ 编排器新增 `_stage_camera_setup` 阶段（bbox→AE 3D 换算出目标点，落 camera_plan.json + camera_setup.jsx）；32 项测试 | 当前会话 |
