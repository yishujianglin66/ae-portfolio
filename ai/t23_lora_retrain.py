# -*- coding: utf-8 -*-
r"""T23: CLIP LoRA大规模重训 — 用50454帧伪标签重训laion CLIP LoRA。

与T7区别:
  - 数据量: 29566→50454帧(T20最新伪标签)
  - 训练轮次: 15→20 epoch(更多数据需更多轮)
  - 输出: clip_lora_aot_v2.pt

产物:
  models/clip_lora_aot_v2.pt — LoRA权重
  reports/t23_lora_report.json — 微调报告
"""
...
def run_t23_lora():
    """主流程"""
    _log("=" * 60)
    _log("T23: CLIP LoRA大规模重训(50454帧)")
    _log("=" * 60)
    ...
    model_path = MODEL_DIR / "clip_lora_aot_v2.pt"
    torch.save({
    ...
    report_path = REPORT_DIR / "t23_lora_report.json"
