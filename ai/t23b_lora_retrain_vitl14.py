# -*- coding: utf-8 -*-
r"""T23b: CLIP ViT-L-14 LoRA对比学习微调 — 基于新底座的大规模重训。

与T23区别:
  - 底座: ViT-B-32 → ViT-L-14 (768维, 4.3亿参)
  - 数据量: 50454帧伪标签 + 3580教师帧
  - 输出: clip_lora_aot_vitl14.pt
...
产物:
  models/clip_lora_aot_vitl14.pt — LoRA权重
  reports/t23b_lora_report.json — 微调报告
"""
...
# LoRA超参(ViT-L-14: 更大模型, 更小batch)
LORA_RANK = 16
LORA_ALPHA = 32
EPOCHS = 20
LR = 2e-5
BATCH_SIZE = 16          # ViT-L-14显存占用更大, 降到16
TEMPERATURE = 0.07
IMG_SIZE = 224           # ViT-L-14 openai用224
EMBED_DIM = 768          # ViT-L-14输出768维


def _log(msg: str):
    print(f"[T23b] {msg}", flush=True)


def load_clip_model():
    """加载laion CLIP ViT-L-14"""
    import open_clip
...
def add_lora_to_clip(clip_model, rank=16, alpha=32):
    """给CLIP ViT-L-14 visual encoder添加LoRA适配器"""
    from peft import LoraConfig, get_peft_model

    # ViT-L-14的attention模块: out_proj, c_fc, c_proj (与ViT-B-32相同结构)
...
def train_lora(clip_model, lora_model, train_loader, text_embeds, device):
    """训练LoRA (ViT-L-14: fp16 autocast节省显存)"""
    ...
    for epoch in range(EPOCHS):
        ...
        for images, labels in train_loader:
            ...
            if n_batches % 100 == 0:
                ...
                _log(f"  [E{epoch+1}] batch {n_batches}/{len(train_loader)} | loss={loss.item():.4f} | ETA {eta/60:.0f}min")

        scheduler.step()
        avg_loss = epoch_loss / max(n_batches, 1)
        history.append({"epoch": epoch + 1, "loss": round(avg_loss, 4)})
        _log(f"  Epoch {epoch+1}/{EPOCHS} | loss={avg_loss:.4f} | 耗时 {(time.time()-t_epoch_start)/60:.1f}min")
        t_epoch_start = time.time()
...
def run_t23b_lora():
    ...
    if device == "cuda":
        ...
        _log(f"VRAM: {vram:.1f}GB")

    # 1. 加载CLIP ViT-L-14
    _log("\n[1/5] 加载CLIP ViT-L-14模型...")
    t0 = time.time()
    clip_model, preprocess, tokenizer = load_clip_model()
    clip_model = clip_model.to(device)
    _log(f"  CLIP ViT-L-14加载完成: {time.time()-t0:.1f}s")
    ...
    model_path = MODEL_DIR / "clip_lora_aot_vitl14.pt"
    torch.save({
        "lora_state": lora_model.state_dict(),
        "class_names": class_names,
        "ip2idx": ip2idx,
        "config": {
            "rank": LORA_RANK, "alpha": LORA_ALPHA, "epochs": EPOCHS,
            "base_model": "ViT-L-14", "embed_dim": EMBED_DIM,
        },
    }, model_path)

    report = {
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "base_model": "laion CLIP ViT-L-14",
        "embed_dim": EMBED_DIM,
    ...
    report_path = REPORT_DIR / "t23b_lora_report.json"
