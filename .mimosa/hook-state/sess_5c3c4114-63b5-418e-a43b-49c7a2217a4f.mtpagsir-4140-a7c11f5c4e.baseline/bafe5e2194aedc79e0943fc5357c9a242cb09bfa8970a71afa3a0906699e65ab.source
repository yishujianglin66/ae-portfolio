...
class UnifiedToolIntegrator:
        ...
        try:
            while pending:
                ...
                if not ready_ids:
                    ...
                    # 将被循环阻塞的步骤如实记入结果（标记 ERROR），
                    # 避免返回空 steps 导致调用方无法诊断失败原因
                    for sid in sorted(pending):
                        step_conf = next(
                            (s for s in steps_config if s["step_id"] == sid), None
                        )
                        blocked = StepResult(
                            step_id=sid,
                            name=step_conf["name"] if step_conf else sid,
                            status=PhaseStatus.ERROR.value,
                            tool=step_conf.get("tool", "") if step_conf else "",
                            operation=step_conf.get("operation", "") if step_conf else "",
                            error="因依赖循环（或依赖不可解）无法执行",
                        )
                        result.steps.append(blocked)
                        step_results_map[sid] = blocked
                        completed.add(sid)
                    pending = set()
                    break
    ...
    def _execute_step(self, step_conf: Dict[str, Any], params: Dict[str, Any]) -> StepResult:
        ...
        # auto 模式诚实降级：real 执行重试耗尽仍失败时，
        # 自动降级到 simulate 再执行一次，避免环境缺输入时整个工作流失败
        if (
            self._default_mode == "auto"
            and result is not None
            and getattr(result, "mode_used", None) == "real"
            and result.status != PhaseStatus.SUCCESS.value
        ):
            self._log(f"步骤 {step_id} auto 模式降级：real 失败，改用 simulate", "WARNING")
            old_mode = adapter.config.mode
            adapter.config.mode = "simulate"
            try:
                fallback = adapter.execute(operation, params)
                fallback.used_fallback = True
                result = fallback
            finally:
                adapter.config.mode = old_mode
        return result
