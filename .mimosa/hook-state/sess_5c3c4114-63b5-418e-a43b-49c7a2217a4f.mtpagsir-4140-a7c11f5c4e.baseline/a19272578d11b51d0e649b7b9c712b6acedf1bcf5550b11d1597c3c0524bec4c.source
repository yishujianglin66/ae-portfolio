...
(function() {
    ...
    var logFile = new File(Folder.myDocuments + "/ae-mcp-bridge/analysis_log.txt");
    ...
    log("Project: " + (app.project.file ? app.project.file.name : "untitled"));
    ...
    log("Total items: " + app.project.numItems);
    ...
    for (var i = 1; i <= app.project.numItems; i++) {
        ...
        log("Analyzing comp: " + item.name + " (layers: " + item.numLayers + ")");
        ...
        for (var li = 1; li <= item.numLayers; li++) {
            ...
            for (var p = 0; p < props.length; p++) {
                try {
                    ...
                    log("Transform error for " + props[p] + ": " + e.message);
                }
            }
                ...
                for (var ei = 1; ei <= layer.Effects.numProperties; ei++) {
                    ...
                    for (var pi = 1; pi <= effect.numProperties; pi++) {
                        try {
                            ...
                            log("Effect param error: " + e.message);
                        }
                    }
    ...
    // Write output
    log("Writing output to: " + outputFile.fsName);
