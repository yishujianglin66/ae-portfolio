...
function applyHorizontalToVerticalV13(comp, layer, ft, cs, sd) {
    ...
    try {
        ...
        try {
            ...
            // 使用与主层相同的源起始时间
            tr.setValueAtTime(cs, layer.property("Time Remap").valueAtTime(cs));
            tr.setValueAtTime(cs + sd, layer.property("Time Remap").valueAtTime(cs + sd));
        } catch(e) {}
...
function applyXenozFastPush(comp, cn, seg) {
    ...
    try {
        ...
        oc.property("Scale").setValueAtTime(seg.start + 1, 1.15);
...
function applyBeatSyncKeyframes(comp, cn) {
    ...
    for (var si = 0; si < SEGMENTS.length; si++) {
        ...
        for (var bi = 0; bi < beats.length; bi++) {
            ...
            // 能量驱动缩放幅度: param = base + range × ln(1+k·energy)/ln(1+k)
            var k = 15;
            var scaleBounce = 100 + 8 * Math.log(1 + k * energy) / Math.log(1 + k);
            // 对所有该段落的图层添加节拍弹跳
            for (var ci = 0; ci < seg.clips.length; ci++) {
...
function applyThreeLayerGrading(comp) {
    ...
    try {
        ...
        // 尝试应用 Kodak LUT (如系统中存在)
        try {
            lumetri.property("LUT File").setValue(LUT_CREATIVE + "Kodak_2383_D55.cube");
        } catch(e) {}
        // 基础校正
        try {
    ...
    for (var si = 0; si < SEGMENTS.length; si++) {
        ...
        try {
            ...
            segAdj.opacity.setValue(40);  // 段落色调不抢主
        } catch(e) {}
    }
}
...
function addCinematicTransitions(comp) {
    ...
    for (var i = 0; i < buildBeats.length; i++) {
        addMicroFlash(comp, buildBeats[i] - 0.03, buildBeats[i] + 0.03, 35);
    }
    ...
    // Drop段: 每隔一拍 RGB故障 + 微闪 (战斗高燃)
    var dropBeats = getBeatsInRange(9, 15);
    for (var i = 0; i < dropBeats.length; i++) {
        if (i % 2 === 0) {
            addRGBGlitch(comp, dropBeats[i], dropBeats[i] + 0.1);
        }
        addMicroFlash(comp, dropBeats[i] + 0.267, dropBeats[i] + 0.297, 50);
    }
...
function getBeatsInRange(startT, endT) {
    var beats = [];
    var first30 = [0.116, 0.697, 1.231, 1.765, 2.299, 2.833, 3.344, 3.878,
                   4.389, 4.923, 5.457, 5.991, 6.525, 7.036, 7.57, 8.104,
                   8.638, 9.172, 9.706, 10.217, 10.751, 11.285, 11.819,
                   12.33, 12.864, 13.398, 13.932, 14.443, 14.977, 15.511];
    for (var i = 0; i < first30.length; i++) {
        if (first30[i] >= startT && first30[i] < endT) {
            beats.push(first30[i]);
        }
    }
    // 补充超出30拍的部分
    var lastBeat = first30[first30.length - 1] + BEAT_INTERVAL;
    while (lastBeat < endT) {
        if (lastBeat >= startT) beats.push(lastBeat);
...
function getEnergyAt(t) {
    ...
    if (idx < 0) return 0.3;
    if (idx >= energyCurve.length) return 0.1;
