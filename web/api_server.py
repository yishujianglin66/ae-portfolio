...
# 统一路径引导：本文件位于 web/，作为脚本运行时 sys.path[0] = web/，
...
# ============================================================================
# 端点收敛：Deprecation 标记中间件（Step 4 Phase B，加法式，不改业务）
# ============================================================================
# 网关 (puppet-automation/src/api/main.py :8000) 为统一执行面真源。
# 本服务中与网关规范端点重复的端点在此集中标记 Deprecation，引导调用方迁移。
# 仅注入响应头，完全不改业务逻辑；如需回退只需清空 _DEPRECATED_PATHS。
...
def _deprecation_hit_logger():
    """Phase C 观测：惰性挂载 JSONL 文件 handler，使 DEPRECATED_HIT 有稳定结构化落盘源，
    不依赖控制台捕获（控制台日志在 Docker/nohup 下易丢失）。落盘失败则 fail-open，绝不阻塞业务。"""
...
_DEPRECATED_SUNSET = (
    _dt.datetime.now(_dt.timezone.utc) + _dt.timedelta(days=180)
).strftime("%a, %d %b %Y %H:%M:%S GMT")

# (prefix, canonical_path) —— canonical 为网关根相对路径（nginx 统一入口）
_DEPRECATED_PATHS: list = [
    ("/metrics", "/metrics"),
    ("/api/v1/resources", "/api/v1/system/resources"),
    ("/api/v1/alerts/active", "/api/v1/system/alerts"),
    ("/api/v1/alerts/history", "/api/v1/system/alerts"),
    ("/api/v1/compose", "/api/v1/ai/plan-and-run"),
    ("/api/v1/tools", "/api/v1/engines"),
]


def _match_deprecated(path: str):
    for prefix, canonical in _DEPRECATED_PATHS:
...
async def health_check_v1():
    """健康检查端点（/api/v1/health 别名）

    与 puppet-automation/src/api/main.py 的 /api/v1/health 路由保持一致，
    供 ae-dashboard 前端（API_BASE=/api/v1）统一访问，避免 404 降级。
    """
...
async def get_system_stats(user: Any = Depends(require_auth)):
    """获取系统统计信息"""
    uptime = time.time() - _start_time
    tasks_stats = {}
    workers = 0
...
async def list_puppet_styles(user: Any = Depends(require_auth)):
    """获取支持的木偶风格列表"""
    return {
        "styles": [
...
async def list_quality_metrics(user: Any = Depends(require_auth)):
    """获取支持的质量评估指标"""
    return {
        "metrics": [
...
async def list_effect_categories(user: Any = Depends(require_auth)):
    """获取效果分类列表"""
    return {
        "categories": [
...
async def list_styles(user: Any = Depends(require_auth)):
    """获取风格模板列表"""
    styles = [
        {
            "id": "cinematic",
...
async def get_style(style_id: str, user: Any = Depends(require_auth)):
    """获取风格模板详情"""
...
async def get_project_detail(project_id: str, user: Any = Depends(require_auth)):
    """获取项目详情"""
...
async def toolchain_status(user: Any = Depends(require_auth)):
    """获取工具链状态"""
...
async def get_resources(user: Any = Depends(require_auth)):
    """获取系统资源信息"""
    try:
...
async def list_plugins(user: Any = Depends(require_auth)):
    """获取插件列表"""
    try:
...
async def get_plugin_stats(user: Any = Depends(require_auth)):
    """获取插件统计信息"""
    try:
        from plugin_system import get_plugin_manager
        manager = get_plugin_manager()
...
try:
    from ai.ai_chat_api import register_ai_chat_routes
...
async def list_tools(user: Any = Depends(require_auth)):
    """获取所有可用工具列表"""
    try:
...
async def get_tool_detail(tool_name: str, user: Any = Depends(require_auth)):
    """获取工具详情"""
    try:
        from tool_executor import get_executor
        executor = get_executor()
...
async def get_tool_history(user: Any = Depends(require_auth)):
    """获取工具调用历史"""
    try:
        from tool_executor import get_executor
        executor = get_executor()
...
async def compose_task(request: CompositionTaskRequest, user: Any = Depends(require_auth)):
...
async def list_compose_tools(user: Any = Depends(require_auth)):
...
async def list_compose_presets(user: Any = Depends(require_auth)):
    """列出所有合成预设"""
    try:
...
async def compose_status(user: Any = Depends(require_auth)):
    """获取编排器状态"""
    try:
        from ae_smart_orchestrator import AESmartOrchestrator
        orch = AESmartOrchestrator()
