...
**⚠️ 诚实边界（并给出下一步）**：本项只解**高光过冲**。交付成片**最突出的观感问题仍是"软"**
（12 帧中 10 帧 Laplacian 0.9–57，正常≈240+）与**饱和度/亮度跨镜摆动**——对应文字线会话的
选项②（逐镜亮度/饱和度归一化）与③（降/统一模糊）。**若继续，建议先攻"软"**（覆盖帧数最多、观感最直接），
其次②。二者均需 Boss 再授权（改 `build_master_polish.py` 的 AE 侧 bloom/Glo2 与逐镜 grade，
或加 ffmpeg 逐镜归一化后级）。
