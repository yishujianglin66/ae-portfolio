...
### §9.4 视觉强化线（第五轮：Boss 授权选项①曝光过冲修正 → 已交付并量化闭环）

Boss 授权"按最推荐步骤推进"（=选项①：曝光/泛光过冲修正 + 一版对比）。**先实测定位，再动手**：

**关键定位（先证伪再修）**：用 12 帧探针对比 **基底母版** `output/unified_run53/run53_premium_final.mp4`
与 **交付成片** `…/text_overlay/render/run53_text_v50_full.mp4`（两者同为 1920×1080/24fps/720 帧/30s，可时间对齐）：
过冲**存在于基底母版**（hi% 峰值 14.7%@0.5s / 11.9%@22.0s / 7.5%@28.0s），
而交付成片 hi% 仅 0–4.4% —— 即文字线会话"成片近全白"的定性，数值上更接近**基底被过冲**、
成片的突出问题其实是**软**（10/12 帧 Laplacian<200）与**饱和度摆动**（0.04↔0.55），后两者属选项②/③。

**交付物（选项①）**：
- 新增 `scripts/grade_highlight_rolloff.py` —— 只压高光、保中间调/黑位/色相的 `curves` 单调软肩后级，
  三档（gentle/medium/strong），默认 **gentle** `all='0/0 0.7/0.7 0.9/0.89 0.97/0.955 1/0.965'`。
  设计对比证据：`min/gentle` 与更钝的 `medium` 同样清零过冲，但锐度损失约减半（8.5s 264→240 vs 225；
  28.0s 131→116 vs 100）；`colorlevels=romax=.94` 亦能清零但会整体压暗中间调，故弃用。
- 已应用到基底母版 → `output/unified_run53/run53_premium_final_hlfix.mp4`（43.2MB，输出目录 gitignored）。
- 对比图（BEFORE|AFTER × 5 个最差帧）：`output/unified_run53/polish/hlfix_compare.png`（1920×2840）。

**量化闭环（12 帧，gentle 档）**：hi% **全部 →0.0**（原 14.7/5.2/3.4/1.8/4.7/11.9/2.8/7.5 …）；
饱和度变化 ≤0.01（几乎不变）；锐度仅 -9%~-12%（8.5s 264→240、28.0s 131→116），其余帧 <5%。
→ 过冲问题**已闭环**；高光处反而**新增了可分辨细节**（死白区原本零细节）。

**⚠️ 诚实边界（含对选项②/③的实测结论）**：本项只解**高光过冲**。对另两项做了实测取证，结论如下——
- **成片级候选已产出**（供 Boss 目视判定，未改管线）：
  `output/unified_run53/polish/run53_v50_polish_candidate.mp4` = 成片 + gentle 高光回收 + cas 轻锐化 + 饱和度×0.97；
  对比图 `output/unified_run53/polish/candidate_compare_film.png`。成片级 hi% 仅 19.0s 有 4.4%→0.0%，其余本就 0 → **选项①对成片增益很小**。
- **②逐镜归一化：对本片不适用**。`production_report.json` 实测 **116 个镜头 / 30s（均长 0.26s）**——快切 AMV，
  单镜太短，逐镜亮度/饱和度归一化无意义。
- **③模糊：后级无法修复**。实测 `cas`（对比自适应锐化）会**放大**跨镜方差（8.5s lap 372→436 而 22.0s 19→18），
  与"统一"目标相反；固定 `unsharp` 会把已锐帧放大到 2.3x（372→849，易出光晕）；而近乎无细节的 0.5s 帧（lap 0.9）
  任何后级都救不回（无中生有）。
- **→ 结论**：成片"软"的根因在**上游**（快切 + 4K 素材慢放/Twixtor 带来的动态模糊 + 镜头选择），
  属**创作/素材层决策**（改镜头选择或 AE 渲染设置），非后级可解，也不宜由执行体臆测 Boss 偏好。
  已交付的确定收益：过冲清零（基底 14.7%→0）+ 可复用的后级工具 + 一版成片候选供判定。

#### 根因精确定位（第五轮续：链路逐段实测，hi% 于 t=0.5/3.3/22.0/28.0）

| 阶段 | t=0.5 | t=3.3 | t=22.0 | t=28.0 |
|---|---|---|---|---|
| `run53_final.mp4`（剪辑） | 0.0 | 0.0 | 0.0 | 0.0 |
| `run53_lut.mp4`（LUT） | 0.0 | 0.0 | 0.0 | 0.0 |
| **`run53_premium_final.mp4`（AE polish）** | **14.7** | **5.2** | **11.9** | **7.5** |
| `text_v50_full.mp4`（交付成片） | 0.0 | 0.0 | 0.0 | 0.0 |

**结论**：过冲**由 `build_master_polish` 的 AE 效果层引入**（剪辑/LUT 都干净），且**在交付成片里已被消除** ——
即选项①的实战对象是 **AE 母版中间产物**，不是观众看到的成片（这解释了"① 对成片增益小"）。

**AE 侧罪魁（plan 证据 `tmp/master_plan.json`）**：被冲爆的镜头挂的是
`radial_soft`（0.25–0.667s, dose 0.857）与 `radial`（21.875–22.125s）= **`CC Radial Fast Blur`**，
且它是**主导效果**（radial_soft 34 + radial 5 = 39/95 镜）；`dose` 范围 0.72–2.42 会把基数 70/35 放大到有效 ~25–85。
**建议（未擅自改，需一次渲染视觉复验）**：下调 `RECIPES["radial"/"radial_soft"]` 的 `CC Radial Fast Blur-0002` 基数，
或改其叠加类型。因这是招牌观感且无法离线复验，留待 Boss 授权一次渲染试验。

**另一定位**：0.5s 开场帧源头 = 初音 Twixtor 片段的**过曝窗口**（源 0–2.4s hi% 4–20%；3–4s 干净 hi%≈0），
换同源素材的非过曝窗口即可，但同样需重渲染。

**落地形态**：`scripts/grade_highlight_rolloff.py` 已重构为**管线可调用**（`build_filter_chain()` /
...
### §9.6 与并行会话的碰撞记录（2026-09-19，重要）

并行会话（文字/视觉线）本日**高度活跃**，已提交：`f06b562` v57 全动力回归、`a7d99a0` v56 回退、
`213ebb2` v55、`118895f` v53/v54、`1ab93ba`/`04f395f` 文字遮挡集成、**`e64ce6d` bulk ruff autofix(~20K)**、
**`7059189` 新增 227 个测试文件**、`1d937db`/`877f2cb` 集成计划评审（判定"不可行/70% 重复/依赖虚构"）、
`a891073` **锐度根因修正**（把我方先前的"10 倍损失"判为测量偏差：跨时间戳无映射 + 黑帧污染中位，真实 ~2x/段）
—— 即 §9.4 里"成片软"的量化口径**已被更正**，后续引用以 `a891073` 为准。

**碰撞事实（两次）**：
1. 我方暂存的 3 个文件（workflow 串行修复 / 本 handoff / esbuild 预算）被其以 **`004c9cc`** 提交
   （提交信息与内容一致，内容完好，非破坏性——即本会话的 CI 修复已落地）。
2. **当前 `ruff.toml` 固定门为红**：`tests/test_filter_engine_characterization.py` 有 **9 处 F821**
   （`FilterDefinition` / `SoftwareAdapter` 未定义）——属**其未提交 WIP**（同目录还有 `_v2.py`，明显在迭代中）。
   **我方未触碰他方 WIP**（遵 §2 T7）。这会让任何"跑 ruff 门即绿"的验证暂时不成立，需等他方收敛。

**⚠️ 方法论教训（补 §9.5）**：全量验证期间存在他方写入 → 运行结果会被污染。本会话 round2/3/5 的卡死
诊断一度被误导：**用 `--timeout=60/30` 做的"切片复现"在 S1 那类 150s 真实编码用例上会产生假"复现"**
（任何 >60s 的合法用例都会触发超时转储）。最终可信证据是：xdist+cov **3/3 卡死** vs 去 cov **1/1 通过**、
