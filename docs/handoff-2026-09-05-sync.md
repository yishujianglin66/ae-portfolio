...
> **v20 修复 (09-05 23:15) 镜头重复出境 — 已交付 `run53_final_curve_v13.mp4` (闸门 7/7)**:
> 用户反馈镜头重复出境。指纹+源窗重叠双重测量定位真因: **无时间窗重叠 (去重 pass 有效),
> 但 top5 源贡献 62% 镜头 — 独自升级5 19 镜吃干 5.7s 源 (整段切碎撒全片), Miku/猫1/猫2/辉夜1
> 各 12 镜吃干 ~4s** — 同场景以不同片段反复出现。修: 源多样化替换 pass — 每源镜头数上限 9,
> 超额普通 S 镜 (22 处: 独自升级5×10, Miku×3, 猫1/猫2/辉夜1×3) 替换为 manifest 未使用源的
> 实测有效内容 (twx 平速曲线覆盖底片, 保留原 fx/zoompan)。替换源池 = 实测通过窗口
> (蓝监 v0300 5/5 ✓, do-you-mean 40s 5/5 ✓, 123.mp4 部分; **枫叶.mov 全黑 / 1(10).mov 全暗 /
> 4._1080p 大部暗 已剔除** — 窗口体检 (亮度 18-225 + 运动 ≥0.3) 为必选门, Nagi 教训固化)。
> 帧条验证替换窗口全为真实新素材。**v12 的三处 closeup 柔化保留**。
