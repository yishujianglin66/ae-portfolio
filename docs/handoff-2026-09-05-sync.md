...
> **⚠️ v12-v14 未提交**: Mimosa L3 钩子拦截全部 git commit (全项目存量 ~972 高危, 与本次改动无关)。
> 已修复 ai/aigc_generator.py (SSRF×6+穿越×2 → 域名白名单+私网拒绝+落盘路径约束, 989→972)。
> **门禁已调为 warn**: `MIMOSA_GIT_GATE_MODE=warn` 已于 09-05 写入用户环境变量 (HKCU\Environment,
> 经注册表确认) — 提交时 L3 扫描仍会运行并打印发现, 但不再阻断。**生效条件: 重启 ZCode**
> (环境变量仅对新启动进程可见); 重启后恢复本会话即可提交。若嫌每次提交扫描慢, 可另设
> `MIMOSA_NO_GIT_GATE=1` 完全关门禁。存量清理 (~972 处, 多为误报) 用 `mimosa audit <dir>` 做专项。
