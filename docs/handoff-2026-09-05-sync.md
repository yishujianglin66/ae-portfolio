...
> 网格偏移 (前后方向不一, 是旋律自身起伏); ③渲染运动剖面发现真因 — **v15 顶点帧实验把定格内容
> 移到运动最高点后, 0.35x 缓爬期的滑动从 MAD 1-2 涨到 12-16**, "停"变成可见的滑, 卡点感知
> = 停止脆度 × 内容静止度, 内容动了就不像卡上。处理: v15 pass 默认关闭 (AEKV_IMPACT_SHIFT=1
> 可复开), 重建恢复引擎所选平静姿态的脆冻结。**教训: 重音感知不止是时刻对, 停的脆度才是可感知性**。
