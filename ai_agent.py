...
# 加载 .env.doubao 配置到环境变量
_env_file = Path(__file__).parent / ".env.doubao"
if _env_file.exists():
    ...
    print(f"[ai_agent] 已加载 .env.doubao 配置")
...
class V4Agent:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if DUCK_MISS_AVAILABLE:
            self.api_key = DUCK_MISS_API_KEY
            self.base_url = DUCK_MISS_BASE_URL
            self.provider = "duckmiss"
            print(f"[V4Agent] 使用 DuckMiss 中转站: {self.base_url}")
        elif ARK_AVAILABLE:
            self.api_key = ARK_API_KEY
            self.base_url = ARK_BASE_URL
            self.provider = "ark"
            print(f"[V4Agent] 使用火山方舟: {self.base_url}")
    ...
    def ask(self, question: str, model: str = "pro", max_tokens: int = 4096) -> str:
    ...
    def _ask_ark(self, prompt: str, model: str = None, system_prompt: str = "", 
        ...
        if "choices" in result and result["choices"]:
            ...
            if "usage" in result:
                ...
                print(f"\n[ARK {model} | {duration:.1f}s | {usage.get('total_tokens', 0)} tokens | ¥{cost:.4f}]")
    ...
    def _calc_cost(self, usage: dict, model: str) -> float:
        """计算费用"""
        if "pro" in model.lower() or "4" in model or "72b" in model.lower() or "32b" in model.lower():
            in_price, out_price = 4, 8
        elif "flash" in model.lower() or "turbo" in model.lower():
