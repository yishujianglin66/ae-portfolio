...
if TYPE_CHECKING:
    # 仅用于类型注解（运行时不导入，避免与 ae.scene_detector 循环导入）
    from ae.scene_detector import SceneCut
...
class SceneDetectResult:
    ...
    def to_scene_cuts(self) -> List["SceneCut"]:
        ...
        无论拿到 List[SceneCut] 还是 SceneDetectResult 都能得到统一格式。

        字段映射（SceneSegment → SceneCut）：
            index / start_time / end_time / duration /
            start_frame / end_frame / frame_count 直接对应；
            thumbnail_frame 默认 0；metadata 默认空 dict。

        Returns:
            List[SceneCut]；若 ae.scene_detector 不可达则返回空列表（优雅降级）。
        """
        import sys
        # 确保项目根目录在 sys.path 中（analysis 包可能先于 ae 包被加载）
        _project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if _project_root not in sys.path:
            sys.path.insert(0, _project_root)
        try:
            # 延迟导入避免与 ae.scene_detector 形成模块级循环
            from ae.scene_detector import SceneCut
        except ImportError:
            return []
