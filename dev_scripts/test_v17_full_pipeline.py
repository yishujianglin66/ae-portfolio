...
def phase2_subtitle_text():
    ...
    try:
        ...
        # 离线环境会降级为 simulated
        record("Whisper V17 转写", result.status == "success",
...
def phase3_davinci():
    ...
    try:
        ...
        if result and hasattr(result, 'status'):
            record("DaVinci 调色执行", result.status == "success",
                   f"status={result.status}")
        elif result:
...
def phase4_opensource_tools():
    ...
    try:
        ...
        record("MoviePy V17 变速1.5x", result.status == "success",
               f"status={result.status}")
        ...
        record("MoviePy V17 缩放480p", result.status == "success",
               f"status={result.status}")
    except Exception as e:
    ...
    try:
        ...
        r = subprocess.run(
            ["ffmpeg", "-y", "-i", V17_VIDEO,
             "-vf", "scale=-1:720",
             "-c:v", "libx264", "-preset", "fast", "-crf", "23", "-an", out_720],
        ...
        r = subprocess.run(
            ["ffmpeg", "-y", "-i", V17_VIDEO, "-t", "3",
             "-vf", "scale=320:-1", "-r", "10", out_gif],
