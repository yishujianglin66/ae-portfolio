...
(function() {
    ...
    var projectDir = scriptPath.parent.parent.fsName; // AE-Knowledge-Vault目录
    var commandFile = new File(projectDir + "/mcp_test_command.json");
    var resultFile = new File(projectDir + "/mcp_test_result.json");
    ...
    function test2_BasicImport() {
        ...
        try {
            var filePath = projectDir + "\\05-测试套件\\test_resources\\test_image.png";
    ...
    function generateReport() {
        ...
        return { text: report, successCount: successCount, failCount: failCount };
    }
    ...
    function writeResult(report) {
        try {
            ...
            resultFile.open("w");
            resultFile.writeln(JSON.stringify(resultObj, null, 2));
            resultFile.close();
            log("✅ 结果已写入: " + resultFile.fsName);
        } catch (e) {
            log("❌ 写入结果失败: " + e.toString());
        }
    }
    ...
    function runAllTests() {
        ...
        log("\n✅ 所有测试完成！结果已保存到 mcp_test_result.json");
    }
