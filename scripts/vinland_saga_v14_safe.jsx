...
//
// V14_safe 核心特性:
// 1. 安全版: 所有 Lumetri 高级属性改为基础效果(Color Balance + HS)
// 2. 新增 HSL 精准青橙调色 → 基础版Color Balance+HS
// 3. 新增 Match Cut 匹配剪辑 + 音效标记系统
// 4. 新增 2.5D 视差增强 + 动态文字系统
// 5. V13.5 全部补偿: 节奏增强/鱼眼增强/音画同步/性能优化
//
// 注意: 此版本为AE 2026兼容版，避免使用Lumetri高级属性导致崩溃
// V13 核心创新:
...
var COMP_NAME = "VinlandSaga_Cinematic_V14";

// V14_safe: 自动渲染配置
var OUTPUT_PATH = "D:/AE-Work/output/";
var OUTPUT_FILE = "VinlandSaga_Battle_V14_safe.mp4";
var AUTO_RENDER = true;  // 是否自动渲染
...
function main() {
    try {
        ...
        try {
            ...
            cam.property("Aperture").setValueAtTime(0, 80);
            cam.property("Aperture").setValueAtTime(9, 100);
            cam.property("Aperture").setValueAtTime(15, 70);
        } catch(e) {}
        ...
        return JSON.stringify({
            ...
            technique: "Cinematic V14_safe - 5 Lens Languages",
            version: "V14_safe",
            outputPath: OUTPUT_PATH + OUTPUT_FILE,
...
function createCinematicSegments(comp) {
    ...
    for (var si = 0; si < SEGMENTS.length; si++) {
        ...
        for (var ci = 0; ci < seg.clips.length; ci++) {
            ...
            // V13.5补偿: srcStart 安全校验（不原地修改 clipInfo，避免二次执行时累积偏移）
            var srcDur = ft ? ft.duration : 0;
            var effectiveSrcStart = clipInfo.srcStart;
            if (srcDur > 0 && effectiveSrcStart + segDur + 0.1 > srcDur) {
                effectiveSrcStart = Math.max(0, srcDur - segDur - 0.1);
            }

            // 时间重映射: 从源视频的 effectiveSrcStart 位置开始播放
            try {
                ly.timeRemapEnabled = true;
                var tr = ly.property("Time Remap");
                tr.setValueAtTime(compStart, effectiveSrcStart);
                tr.setValueAtTime(compStart + segDur, effectiveSrcStart + segDur);
            } catch(e) {}
...
function applyThreeLayerGrading(comp) {
    ...
    try {
        ...
        if (lutExists) {
            ...
            // V14_safe: 回退方案 - 用 Brightness & Contrast + Color Balance 模拟胶片感
            try {
                var bc = master.property("Effects").addProperty("Brightness & Contrast");
                bc.property("Contrast").setValue(20);
                bc.property("Brightness").setValue(-3);
            } catch(e) {}
            try {
                var cb = master.property("Effects").addProperty("ADBE Color Balance");
                cb.property("Shadow Red Balance").setValue(-5);
                cb.property("Shadow Blue Balance").setValue(10);
                cb.property("Highlight Red Balance").setValue(10);
                cb.property("Highlight Blue Balance").setValue(-8);
            } catch(e) {}
        }
        // V14_safe: 基础校正 (Brightness & Contrast)
        try {
            var bc2 = master.property("Effects").addProperty("Brightness & Contrast");
            bc2.property("Contrast").setValue(12);
        } catch(e) {}
    } catch(e) {}
    ...
    // ===== Layer 3: 段落级色调 (V14_safe: Color Balance + Hue/Saturation) =====
    for (var si = 0; si < SEGMENTS.length; si++) {
        var seg = SEGMENTS[si];
        try {
            // V14_safe: 段落级色调 - 使用 Color Balance + Hue/Saturation (安全可靠)
            ...
            var segCB = segAdj.property("Effects").addProperty("ADBE Color Balance");
            var segHS = segAdj.property("Effects").addProperty("ADBE Hue & Saturation");
            
            if (seg.name === "Intro") {
                segCB.property("Shadow Red Balance").setValue(-10);
                segCB.property("Shadow Green Balance").setValue(3);
                segCB.property("Shadow Blue Balance").setValue(18);
                segCB.property("Midtone Red Balance").setValue(-8);
                segCB.property("Midtone Blue Balance").setValue(12);
                segCB.property("Highlight Red Balance").setValue(-3);
                segCB.property("Highlight Blue Balance").setValue(8);
                segHS.property("Colorize Hue").setValue(200);
                segHS.property("Colorize Saturation").setValue(15);
            } else if (seg.name === "Build") {
                segCB.property("Shadow Red Balance").setValue(10);
                segCB.property("Shadow Green Balance").setValue(5);
                segCB.property("Shadow Blue Balance").setValue(-12);
                segCB.property("Midtone Red Balance").setValue(15);
                segCB.property("Midtone Green Balance").setValue(8);
                segCB.property("Midtone Blue Balance").setValue(-10);
                segCB.property("Highlight Red Balance").setValue(18);
                segCB.property("Highlight Green Balance").setValue(10);
                segCB.property("Highlight Blue Balance").setValue(-5);
                segHS.property("Colorize Hue").setValue(30);
                segHS.property("Colorize Saturation").setValue(20);
            } else if (seg.name === "Drop") {
                segCB.property("Shadow Red Balance").setValue(20);
                segCB.property("Shadow Green Balance").setValue(2);
                segCB.property("Shadow Blue Balance").setValue(-20);
                segCB.property("Midtone Red Balance").setValue(25);
                segCB.property("Midtone Green Balance").setValue(-5);
                segCB.property("Midtone Blue Balance").setValue(-25);
                segCB.property("Highlight Red Balance").setValue(15);
                segCB.property("Highlight Green Balance").setValue(-8);
                segCB.property("Highlight Blue Balance").setValue(-15);
                segHS.property("Colorize Hue").setValue(0);
                segHS.property("Colorize Saturation").setValue(25);
            } else if (seg.name === "Break") {
                segCB.property("Shadow Red Balance").setValue(-15);
                segCB.property("Shadow Green Balance").setValue(3);
                segCB.property("Shadow Blue Balance").setValue(25);
                segCB.property("Midtone Red Balance").setValue(-12);
                segCB.property("Midtone Green Balance").setValue(5);
                segCB.property("Midtone Blue Balance").setValue(18);
                segCB.property("Highlight Red Balance").setValue(-8);
                segCB.property("Highlight Green Balance").setValue(2);
                segCB.property("Highlight Blue Balance").setValue(12);
                segHS.property("Colorize Hue").setValue(220);
                segHS.property("Colorize Saturation").setValue(10);
            } else {
                segCB.property("Shadow Red Balance").setValue(-5);
                segCB.property("Shadow Green Balance").setValue(3);
                segCB.property("Shadow Blue Balance").setValue(12);
                segCB.property("Midtone Red Balance").setValue(5);
                segCB.property("Midtone Green Balance").setValue(7);
                segCB.property("Midtone Blue Balance").setValue(-5);
                segCB.property("Highlight Red Balance").setValue(10);
                segCB.property("Highlight Green Balance").setValue(10);
                segCB.property("Highlight Blue Balance").setValue(-6);
                segHS.property("Colorize Hue").setValue(30);
                segHS.property("Colorize Saturation").setValue(15);
            }
    ...
    // V14: 应用 HSL Secondary 精准青橙
    applyPreciseCyanOrange(comp);
}
...
function addCinematicTransitions(comp) {
    ...
    // V14: Drop→Break Match Cut 匹配剪辑 (动作匹配)
    addMatchCut(comp, 14.9, {src:"FIGHT_S2"}, {src:"MUKANJYO"}, "action");
...
// V14: 2.5D 视差分层增强
function apply25DParallax(comp) {
    try {
        for (var i = 1; i <= comp.numLayers; i++) {
            var ly = comp.layer(i);
            if (ly.name.indexOf("_BG") !== -1) {
                ly.zPosition.setValueAtTime(0, 500);
                ly.zPosition.setValueAtTime(DURATION, 300);
            } else if (ly.name.indexOf("_BG") === -1 && ly.name !== "PRECOMP_Backgrounds" && ly.name.indexOf("FX_") === -1) {
                ly.zPosition.setValueAtTime(0, 100);
                ly.zPosition.setValueAtTime(DURATION, 150);
            }
        }
        
        try {
            var cam = comp.layer("Camera_1");
            cam.property("Z Position").setValueAtTime(0, -500);
            cam.property("Z Position").setValueAtTime(DURATION / 2, -400);
            cam.property("Z Position").setValueAtTime(DURATION, -500);
            
            cam.property("X Position").setValueAtTime(0, 0);
            cam.property("X Position").setValueAtTime(DURATION / 4, 50);
            cam.property("X Position").setValueAtTime(DURATION / 2, 0);
            cam.property("X Position").setValueAtTime(DURATION * 0.75, -50);
            cam.property("X Position").setValueAtTime(DURATION, 0);
        } catch(e) {}
    } catch(e) {}
}
...
// V14_safe: 青橙调色增强 (Color Balance + Hue/Saturation + 遮罩)
function applyPreciseCyanOrange(comp) {
    try {
        var adj = comp.layers.addSolid([1,1,1], "FX_Precise_CyanOrange", WIDTH, HEIGHT, 1);
        adj.adjustmentLayer = true;
        adj.startTime = 0; adj.inPoint = 0; adj.outPoint = DURATION;
        
        var cb = adj.property("Effects").addProperty("ADBE Color Balance");
        cb.property("Shadow Red Balance").setValue(-12);
        cb.property("Shadow Green Balance").setValue(5);
        cb.property("Shadow Blue Balance").setValue(20);
        cb.property("Midtone Red Balance").setValue(-5);
        cb.property("Midtone Blue Balance").setValue(10);
        cb.property("Highlight Red Balance").setValue(15);
        cb.property("Highlight Green Balance").setValue(8);
        cb.property("Highlight Blue Balance").setValue(-10);
        
        var hs = adj.property("Effects").addProperty("ADBE Hue & Saturation");
        hs.property("Colorize Hue").setValue(15);
        hs.property("Colorize Saturation").setValue(10);
        hs.property("Lightness").setValue(-3);
        
        var bc = adj.property("Effects").addProperty("Brightness & Contrast");
        bc.property("Contrast").setValue(10);
        
        adj.opacity.setValue(75);
    } catch(e) {}
}
...
// V14: Match Cut 匹配剪辑
function addMatchCut(comp, t, clipInfo1, clipInfo2, matchType) {
    try {
        var mc = comp.layers.addSolid([1,1,1], "MatchCut_" + t, WIDTH, HEIGHT, 1);
        mc.adjustmentLayer = true;
        mc.startTime = t - 0.1; mc.inPoint = t - 0.1; mc.outPoint = t + 0.2;
        
        if (matchType === "action") {
            var dirBlur = mc.property("Effects").addProperty("ADBE Directional Blur");
            dirBlur.property("Blur Length").setValueAtTime(t - 0.1, 0);
            dirBlur.property("Blur Length").setValueAtTime(t, 300);
            dirBlur.property("Blur Length").setValueAtTime(t + 0.1, 0);
            dirBlur.property("Direction").setValue(0);
            
            mc.opacity.setValueAtTime(t - 0.1, 0);
            mc.opacity.setValueAtTime(t - 0.02, 100);
            mc.opacity.setValueAtTime(t + 0.02, 100);
            mc.opacity.setValueAtTime(t + 0.1, 0);
        } else if (matchType === "composition") {
            var radial = mc.property("Effects").addProperty("ADBE Radial Blur");
            radial.property("Amount").setValueAtTime(t - 0.08, 0);
            radial.property("Amount").setValueAtTime(t, 50);
            radial.property("Amount").setValueAtTime(t + 0.08, 0);
            radial.property("Type").setValue(1);
            
            mc.opacity.setValueAtTime(t - 0.08, 0);
            mc.opacity.setValueAtTime(t - 0.01, 80);
            mc.opacity.setValueAtTime(t + 0.01, 80);
            mc.opacity.setValueAtTime(t + 0.08, 0);
        }
    } catch(e) {}
...
function addSoundMarker(comp, t, soundType) {
    try {
        comp.markerTracks.addMarker(t);
        var marker = comp.markerTracks.marker(t);
        marker.name = "SFX_" + soundType;
        marker.comment = "Sound effect: " + soundType;
    } catch(e) {}
}

function addBeatSoundMarkers(comp) {
    var beatTypes = ["whoosh", "impact", "swoosh", "glitch"];
    var dropBeats = getBeatsInRange(9, 15);
    for (var i = 0; i < dropBeats.length; i++) {
        var type = beatTypes[i % beatTypes.length];
        addSoundMarker(comp, dropBeats[i], type);
    }
    
    var buildBeats = getBeatsInRange(4, 9);
    for (var j = 0; j < buildBeats.length; j++) {
        addSoundMarker(comp, buildBeats[j], "swoosh");
    }
    
    addSoundMarker(comp, 9, "impact");
    addSoundMarker(comp, 15, "cinematic");
    addSoundMarker(comp, 23.15, "cinematic");
}
