...
bge_command = [
    "py", "-3.12", "-c",
    "import sys,os; sys.path.insert(0,'.'); os.environ['PYTHONDONTWRITEBYTECODE']='1'; from ai.t11_bge_m3_search import BGESearcher; s=BGESearcher(); vecs=s.encode(['测试句子1','这是向量搜索的验证','科研级封口双重验证'], max_length=512); print(f'T13-INDEPENDENT-VALIDATION dim={vecs.shape} dtype={vecs.dtype} sum={float(vecs.sum()):.6f} norm={float((vecs**2).sum()**0.5):.6f}'); import json; print(json.dumps({'shape': list(vecs.shape), 'dtype': str(vecs.dtype)}))"
]
...
print(f"  stdout: {stdout_full[:500]}{'...(truncated)' if len(stdout_full) > 500 else ''}")
print(f"  stderr: {stderr_full[:500]}{'...(truncated)' if len(stderr_full) > 500 else ''}")
...
ssl_error_keywords = ["UNEXPECTED_EOF_WHILE_READING", "client has been closed", "SSL", "ssl"]
