...
def generate_blender_import_script(
    ...
    for i, info in enumerate(layers_info):
        ...
        lines.extend([
            ...
            f"if os.path.isfile(img_path):",
            f"    try:",
            f"        img = bpy.data.images.load(img_path, check_existing=True)",
            f"        bpy.ops.mesh.primitive_plane_add(size=1, location=(0, 0, {-i * 0.01:.3f}))",
            f"        plane = bpy.context.active_object",
            f'        plane.name = "{name}"',
            f"        sx = {w / max(w, h):.4f}",
            f"        sy = {h / max(w, h):.4f}",
            f"        plane.scale = (sx, sy, 1)",
            f"",
            f"        # 创建材质",
            ...
            f"",
            f"        # 创建输出节点",
            f'        output_node = nodes.new("ShaderNodeOutputMaterial")',
            f"        output_node.location = (300, 0)",
            f"",
            f"        # 创建 BSDF 节点",
            f'        bsdf = nodes.new("ShaderNodeBsdfPrincipled")',
            f"        bsdf.location = (0, 0)",
            f"",
            f"        # 创建纹理节点",
            f'        tex = nodes.new("ShaderNodeTexImage")',
            f"        tex.location = (-300, 0)",
            f"        tex.image = img",
            f"",
            f"        # 连接",
            f"        links.new(tex.outputs['Color'], bsdf.inputs['Base Color'])",
            f"        links.new(bsdf.outputs['BSDF'], output_node.inputs['Surface'])",
            f"",
            f"        # 透明混合",
            f"        mat.blend_method = 'CLIP'",
