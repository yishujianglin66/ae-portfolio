...
请生成完整剪辑剧本JSON。"""
    ...
    def _fallback_script(self, prompt: str, analyses: List[Dict],
        ...
        seg_types = ["intro", "build", "drop", "break", "outro"]
        seg_dur = total_dur / len(seg_types)
        segments = []
        for i, st in enumerate(seg_types):
            ...
            segments.append({
                ...
                "mood": {"intro": "铺垫", "build": "升温", "drop": "高潮",
                         "break": "沉淀", "outro": "收尾"}.get(st, "中性"),
                "camera": {
                    "movement": {"intro": "推", "build": "摇", "drop": "快推",
                                 "break": "拉", "outro": "慢拉"}.get(st, "推"),
                    "speed": {"intro": "slow", "build": "normal", "drop": "fast",
                              "break": "slow", "outro": "slow"}.get(st, "normal"),
                    "technique": f"{st}_camera"
                },
                "text_overlay": {
                    "text": {"intro": "VINLAND SAGA", "build": "战斗开始",
                    ...
                    "animation": {"intro": "fade_in", "build": "slide_up",
                                  ...
                                  "outro": "fade_in"}.get(st, "fade_in"),
                },
                "effects": ["Lumetri Color", "Glow"] if st == "drop" else ["Lumetri Color"],
                "transition_to_next": {
