...
使用方式：
    ...
    # 指定模型
    result = client.ask("专业问题", model="doubao-seed-2-1-pro")
"""
...
MODEL_TIER_MAP = {
    ...
    "doubao-seed-2-1-pro": ModelTier.PRO,
    ...
    "doubao-seed-2-1-turbo": ModelTier.FLASH,
...
"""
    ...
    def qa(self, question: str, use_turbo: bool = False) -> str:
        if use_turbo:
            return self.ask(question, model="doubao-seed-2-1-turbo")
        ...
        if pro_score > turbo_score:
            return self.ask(question, model="doubao-seed-2-1-pro")
        elif turbo_score >= 2:
            return self.ask(question, model="doubao-seed-2-1-turbo")
        else:
            return self.ask(question, model="doubao-seed-2-1-pro")
...
语气要温暖、鼓励、专业。"""
        ...
        return self.ask(prompt, model="doubao-seed-2-1-pro", max_tokens=8192)
